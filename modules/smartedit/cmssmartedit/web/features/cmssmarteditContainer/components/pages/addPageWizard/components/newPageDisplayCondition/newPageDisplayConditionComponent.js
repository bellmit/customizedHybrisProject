...
angular.module('newPageDisplayConditionModule', ['pageServiceModule', 'pageDisplayConditionsServiceModule'])
    ...
    .controller('newPageDisplayConditionController', function(
        ...
        this._getSelectedPrimaryPageAndDisplayCondition = () => {
            if (this.pageTypeCode && this.pageUuid) {
                pageService.getPageByUuid(this.pageUuid).then((page) => {
                        // defaultPage means whether the page is a Primary Page
                        const primaryPagePromise = (!page.defaultPage ?
                            pageService.getPrimaryPage(page.uid) :
                            $q.resolve(page));
                        return primaryPagePromise.then((primaryPage) => {
     ...
     * @param {< String} pageUuid Uuid of the page being cloned. Used to resolve the Primary Page that is pre-selected.
