...
angular
    ...
    .run(function(
        ...
        function registerRules() {
            permissionService.registerRule({
                ...
                verify: function(permissionNameObjs) {
                    var promises = permissionNameObjs.map(function(
                ...
                verify: function(permissionNameObjs) {
                    function isAvailableForCurrentPrincipal(workflow) {
                        return workflow === null ?
                            true :
                            workflow.isAvailableForCurrentPrincipal;
                    }
            ...
            permissionService.registerRule({
                ...
                verify: function(permissionNameObjs) {
                    function isUserParticipant(workflow) {
                        return workflow === null ?
                            $q.when(true) :
                            workflowService.isUserParticipanInActiveAction(
        ...
        function registerRulesForTypeCodeFromContext() {
            var registerTypePermissionRuleForTypeCodeFromContext = function(
                ruleName,
                verify
            ) {
                permissionService.registerRule({
                    ...
                    verify: function(permissionNameObjs) {
                        var promises = permissionNameObjs.map(function(
                            ...
                            return verify([permissionNameObject.context.typeCode]).then(
                                function(UpdatePermission) {
                                    return UpdatePermission[
                                        permissionNameObject.context.typeCode
                                    ];
                                }
                            );
                        });
        ...
        function registerRulesForCurrentPage() {
            var registerTypePermissionRuleOnCurrentPage = function(
                ruleName,
                verify
            ) {
                permissionService.registerRule({
                    ...
                    verify: function() {
                        return pageService.getCurrentPageInfo().then(function(pageInfo) {
                            return verify([pageInfo.typeCode]).then(function(
                                permissionObject
                            ) {
        ...
        function registerRulesForTypeCode() {
            var registerTypePermissionRuleForTypeCode = function(
                ruleName,
                itemType,
                verify
            ) {
        ...
        function registerPermissions() {
            ...
            permissionService.registerPermission({
                aliases: [
                    "se.slot.context.menu.shared.icon",
                    "se.slot.context.menu.unshared.icon",
                ],
