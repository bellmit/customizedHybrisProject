...
angular
    ...
    .run(
        function($httpBackend, parseQuery, backendMocksUtils, navigationNodePOST, navigationNodeNodes) {
            ...
            // pages mock
            $httpBackend.whenGET(/\/cmswebservices\/v1\/sites\/apparel-uk\/catalogs\/apparel-ukContentCatalog\/versions\/Online\/pages\/.*$/).respond(function(method, url, data, headers) {
                var pageUuid = /pages\/(.*)/.exec(url)[1];
                var page = JSON.parse(sessionStorage.getItem('componentMocks')).componentItems.find(function(item) {
                    return item.uuid === pageUuid;
                });
                return [200, page];
            });
