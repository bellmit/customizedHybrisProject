...
describe("Test WorkflowFacade", () => {
	...
	describe('start workflow ', () => {
		...
		it('WHEN workflow is started THEN it relevant event is published', () => {
			workflowFacade.startWorkflow();

			expect(systemEventService.publish).toHaveBeenCalledWith(WORKFLOW_CREATED_EVENT);
		});

		it('WHEN workflow is started THEN it updates workflow tasks count', () => {
	...
	describe('cancel workflow', () => {
		...
		it("WHEN workflow is canceled THEN confirmation message is displayed AND success alert is shown AND relevant event is published", () => {
		...
		it('WHEN workflow is canceled THEN an event regarding workflow task count update is published', () => {
			const updateWorkflowTasksCountSpy = spyOn(workflowFacade, 'updateWorkflowTasksCount');
